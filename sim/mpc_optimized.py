...
def get_chunk_size(quality, index):
    ...
    return size_video_array[quality, index % TOTAL_VIDEO_CHUNK]
...
def calculate_rebuffer(future_chunk_length, buffer_size, bit_rate, last_index,
                       future_bandwidth, CHUNK_COMBO_OPTIONS):
...
def main():
    ...
    if args.env_random_start:
        ...
        net_env = env.EnvironmentNoRandomStart(all_cooked_time=all_cooked_time,
    ...
    while True:  # serve video forever
        ...
        last_index = int(net_env.total_video_chunk - video_chunk_remain) - 1
        # import ipdb; ipdb.set_trace()
        future_chunk_length = MPC_FUTURE_CHUNK_COUNT
        if (net_env.total_video_chunk - last_index - 1 < MPC_FUTURE_CHUNK_COUNT):
            future_chunk_length = net_env.total_video_chunk - last_index - 1

        # all possible combinations of 5 chunk bitrates (9^5 options)
        # iterate over list and for each, compute reward and store max reward
        # combination
        # start = time.time()
        chunk_combo_options = np.array(CHUNK_COMBO_OPTIONS)

        # old_bitrate = bit_rate
        bit_rate = calculate_rebuffer(future_chunk_length, buffer_size,
                                      bit_rate, last_index, future_bandwidth,
                                      chunk_combo_options)
