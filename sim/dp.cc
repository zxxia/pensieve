...
ALL_COOKED_TIME_BW get_all_cooked_time_bw(std::string trace_dir) {
    ...
    if (dir != NULL) {
        while ((entry = readdir(dir)) != NULL) {
            ...
            std::string file_name =  trace_dir + "/" + entry->d_name;
...
std::vector<std::vector<unsigned int>> get_video_sizes(std::string video_size_dir) {

    std::vector<std::vector<unsigned int> > video_sizes;
    for(int bit_rate = 0; bit_rate < BITRATE_LEVELS; bit_rate ++) {
        std::ifstream in_file(video_size_dir + "/video_size_" + std::to_string(bit_rate));
        std::string line;
...
int main(int argc, char** argv) {
    if (argc != 4){
        ...
        std::cout << "Usage: ./dp [trace_dir] [output_dir] [video_size_dir]" <<
            std::endl;
        return -1;
    }
    std::string trace_dir = argv[1];
    std::string output_dir = argv[2];
    std::string video_size_dir = argv[3];
    std::cout << "trace_dir: " << trace_dir << std::endl <<
        "output_dir: " <<  output_dir << std::endl <<
        "video_size_dir: " << video_size_dir << std::endl;

    ALL_COOKED_TIME_BW all_cooked_time_bw = get_all_cooked_time_bw(trace_dir);
    std::vector<std::vector<unsigned int>> video_sizes = get_video_sizes(video_size_dir);
    ...
    for (unsigned int trace_idx = 0;
        ...
        log_file.open(output_dir + "/log_sim_dp_" + all_cooked_time_bw.all_file_names[trace_idx]);
